...
export class ClasseRepository extends Repository<ClasseEntity> {
    ...
    async getAllClasseByTrainerId(trainerId: string, pagOpts: Partial<PaginateParams>): Promise<EntityResults<ClasseEntity>> {
        try {
            const [entities, count] = await this.getAllClassesQueryBuilder(pagOpts)
    ...
    async getAllClasses(pagOpts: Partial<PaginateParams>): Promise<EntityResults<ClasseEntity>> {
        try {
            const [entities, count] = await this.getAllClassesQueryBuilder(pagOpts)
              .getManyAndCount();
            return { entities, count };
